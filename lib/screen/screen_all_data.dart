...
class _HomeScreenState extends State<ScreenAllData> {
  ...
  double default_radius = 950;
  String api_url = "";

  void _firstLoad() async {
    api_url = "http://geohazard.igtl.tl:83/api/events/paginate/100/radius/$default_radius?page=1";
    setState(() {
      _isFirstLoadRunning = true;
    });

    print("default radius : $default_radius");

    try {
      evento ??= await _service.fetchData(api_url);
      print("evento!.nextPageUrl: ${evento!.nextPageUrl}");
      setState(() {
        // if next page url is null, then assign default api url
        if (evento!.nextPageUrl == null) {
          api_url = "http://geohazard.igtl.tl:83/api/events/paginate/100/radius/$default_radius?page=1";
          datums = [];
        } else {
  ...
  void _loadMore() async {
    print("load more default radius: $default_radius");
  ...
  Future<void> _OnMarkerClickShowDialog(Datum item) async {
  ...
  Widget renderTabList(BuildContext context) {
    ...
    var bodySize = screenSize.height - (screenSize.height * 0.25);
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      Padding(
                          ...
                          child: Text('Radius: ${default_radius.round()} km', style: const TextStyle(color: Colors.white70),)
                      )
                    ],
                  )
        ...
        ? Center(
            child: Column(
              ...
              children: [
                ...
                Text(
                  "Fetching data from IGTL for radius ${default_radius.round()} km",
                )
              ],
            ),
          )
    ...
    if (datums!.isEmpty) {
      return const SizedBox(
        ...
        child: Column(
          ...
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Text("No data is retrieved!\nResource not available! Please try again later.",
